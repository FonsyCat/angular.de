...
Dieses Tutorial zeigt die die grundlegenden Bestandteile einer Angular Anwendung anhand eines praktischen Beispiels, welches du selber implementieren oder fertige Musterlösungen nutzen und verändern kannst.
...
Google nutzt Angular nach eigenen Angaben selbst in über 1600 Projekten.
...
Das Ökosystem von Angular ist sehr groß. Die Basis bildet hierbei das Core-Framework. Hier sind die fundamentalen Konzepte implementiert, die für moderne Web-Anwendungen essentiell sind. Zwei weitere Core-Konzepte - die jedoch separat nutzbar sind - sind die Angular-CLI und die Verwaltung von Komponenten. Diese bilden die Kernfunktionalitäten ab, welche in fast jeder Anwendung benötigt werden. Weitere Module lassen sich *optional einbinden*, falls ihr diese benötigt:
...
Die Angular-CLI wird genutzt, um neue Strukturen innerhalb unserer Anwendungen zu generieren, anstatt wie oft in Projekten die Basis-Strukturen zu kopieren und über potentielle Fehler bei der Umbenennung zu stolpern. Es ist ein mächtiges Werkzeug, welches euch mit `ng  --help` einen ausführlichen Hilfetext anbietet.
...
Eure Basisanwendung ist nun generiert und kann im Browser unter http://localhost:4200 aufgerufen werden. Ihr solltet eine Ähnliche wie im folgenden Bild sehen:
...
* Einem **Selektor**, welcher das HTML-Element beschreibt, welches wir erzeugen.
...
Der Serve-Prozess erkannt aber automatisch Veränderungen innerhalb eures Quellcode und kompiliert die jeweils aktuelle Version der Anwendung in wenigen Sekunden.
...
Hier sehen wir, wie erwartet, eine Komponente. Unser Selektor hat den automatischen Präfix `app-` bekommen. Somit ist unsere neue Komponente nun unter dem HTML-Tag `<app-info-box></app-info-box>` nutzbar. Der Einstiegspunkt unserer kompletten Anwendung ist ebenfalls eine Komponente mit dem Namen `AppComponent`.
...
Diese werden mit doppelten geschweiften Klammern geöffnet und auch wieder geschlossen.

   {{ expression }}

Eine Expression wird von Angular dynamisch auf Basis der aktuellen Properties eurer Klasse ausgewertet.
Führen wir also eine neue Property `text` ein und füllen diese mit einem String, können wir diesen in unserem Template ausgeben.
...
Um auf eine Property eines Elementes zuzugreifen, nutzen wir die eckigen Klammern innerhalb unseres HTML Templates. Möchten wir also z.B. die [HTMLElement.hidden Property](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/hidden) einer Komponente beeinflussen, können wir das wie folgt erreichen:
...
Wir können natürlich auch jedes andere Event wie z.B. `keyup` benutzen. Mit diesem sehr simplen Mechanismus können wir generisch alle Arten von Komponenten benutzen und mit ihnen interagieren. Dies ist unabhängig davon, ob sie in Angular oder einem anderem Framework geschrieben sind.
...
Strukturelle Direktiven werden mit dem Präfix `*` gekennzeichnet.

Die Direktive ist angelehnt an eine For-Schleife und iteriert über eine listen-artige Struktur und erzeugt für jedes Element eine Kopie des DOM Elements auf das es angewandt wird.
...
Hierbei wird eine sogenannte `Looping Variable`, in unserem Beispiel `book` und eine Liste `books` definiert. Die Variable `book` enthält somit jeweils den Wert des aktuellen Listeneintrags.
...
Ein Service sollte sich immer um eine explizite Aufgabe kümmern, und dementsprechend auch danach benannt werden.
In unserem Fall wollen wir die Daten von Büchern verwalten.
...
Der Service bietet uns die Methode `.get(url:string)`, welche wir dem API-Endpoint für unsere Abfrage angeben können. Wir nutzen hier die Adresse des lokal gestarteten JSON-Server.
...
Der Aufruf innerhalb unserer Komponente ändert sich also im Grunde nicht. Jedoch muss die Auswertung innerhalb unseres HTML-Templates etwas angepasst werden. Mit Hilfe einer sogenannten `async` Pipe können wir der `*ngFor` Direktive den Umgang mit der asynchronen Datenstruktur ermöglichen.
...
Die `async` Pipe in Verbindung mit `*ngFor` registriert sich auf asynchrone Updates der `books$` Variable. Durch diese Anpassung unseres Templates können wir nun auch die Daten von unseren JSON-Server wie folgt anzeigen lassen:
